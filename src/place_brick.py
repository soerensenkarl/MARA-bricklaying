# place_brick.py - Place a brick at (x,y,z) with optional yaw (deg) relative to an AprilTag fiducial.
# - Walks Spot's base to x-0.8 (in fiducial frame), heading aligned to fiducial +X.
# - Then executes a short arm motion sequence to place the brick with desired yaw about +Z.
#
# Usage (from sequence.py):
#     import place_brick
#     place_brick.run(robot, target=(x, y, z), yaw_deg=0.0)
...
SECONDS_PER_STEP = 3.0          # Default arm move time (Increased for precision)
...
def _preplace_move_base(command_client, used_T_fid: SE3, x: float, y: float, frame_name: str):
    """Walk base to (x + PREPLACE_X_OFFSET, y) in fiducial XY, with heading EXACTLY
    aligned to the fiducial +X. This intentionally ignores any target brick yaw."""

    # Goal: offset purely along fiducial +X from the brick's (x, y)
    goal_fid = SE3(x + PREPLACE_X_OFFSET, y, 0.0, Quat())

    # Express that goal in the command frame we are using for the base move
    goal_in_used = used_T_fid * goal_fid
    gx, gy = goal_in_used.x, goal_in_used.y

    # Heading: strictly the direction of fiducial +X, expressed in the command frame
    # (independent of any brick rotation)
    ...
    print(f"[pre-place] Walking base to ({gx:.3f}, {gy:.3f}) m in {frame_name}, "
          f"heading={math.degrees(heading):.1f} deg (aligned to fiducial +X)")

    # Build and send the base trajectory. No follow-arm here, so yaw won't be influenced by the arm.
...
def run(robot, target: Tuple[float, float, float], *, yaw_deg: float = 0.0):
    """Place a brick at (x,y,z) relative to TAG_ID fiducial with an added yaw (deg) about +Z."""
    ...
    try:
        ...
        print(f"Error: {e}. Cannot start arm sequence.")
        return
    ...
    print(f"Locked on {fid_name} in {frame_for_arm_cmds}. Starting arm sequence (yaw={yaw_deg:.1f} deg).")

    # Arm steps.
    steps = lay_brick(x, y, z)
    for i, step in enumerate(steps, start=1):
        # Gripper-only shorthand
        ...
        print(
            ...
            f"in {frame_for_arm_cmds} pitch={pitch_deg:.1f} deg, yaw={yaw_deg:.1f} deg, "
